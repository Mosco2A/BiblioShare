...
**Ce qui a été fait :**
- Fix bug profil bloquant la NavigationBar : remplacé `SliverFillRemaining` + `Spacer` par `SliverToBoxAdapter` + spacing fixe
- Ajouté `FloatingActionButton` caméra dans l'onglet Bibliothèque quand des livres sont présents (accès scan permanent)
- Créé test harness (`test_app.dart`) pour contourner Firebase Auth en tests (forceAuthenticatedForTest)
- Écrit 11 tests d'acceptance utilisateur couvrant tous les parcours : home, bibliothèque, scanner, social, profil, navigation, détail livre, thème
- Fix `setState during build` dans HomeScreen (addPostFrameCallback)
- Fix `userId` null en tests (ajout `_testUserId` dans AuthProvider)
- Tous les 17 tests passent (11 acceptance + 6 originaux)

**Décisions :**
- Test harness séparé pour éviter la dépendance Firebase Auth (anonymous auth désactivé dans le projet)
- `findsWidgets` au lieu de `findsOneWidget` pour les titres SliverAppBar.large (render 2x : expanded + collapsed)
- PopupMenu vérifie les vrais textes : Modifier, Prêter, Recommander, Supprimer + boutons "Noter / Avis" et "Prêter"

**Problèmes résolus :**
- Profil bloquait la navigation : `SliverFillRemaining` poussait le contenu et couvrait la NavigationBar
- Pas de bouton scan en bibliothèque quand des livres existent (seulement dans l'empty state)
